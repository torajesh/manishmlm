...
$login_qry=mysqli_query($con,"SELECT * from admin_login where email='".trim($_POST['email'])."' and password='".trim($_POST['password'])."' and status='Active' ");
$cnums=mysqli_num_rows($login_qry);
if($cnums>0){
    $row=mysqli_fetch_array($login_qry);
    $_SESSION['MANMID']=$row['id'];  
    $_SESSION['MANM_NAME'] = $row['contact_name'];
    setcookie("MANM_NAME",$row['contact_name'],time()+107500);
    setcookie("MANM_MOBILE",$row['mobile_phone'],time()+107500);
    header('location:Dashboard.php');
    } else {
        $msg = "<div class='alert alert-danger'>You have entered invalid Email and Password</div>"; 
    }	
}
?>
<!DOCTYPE html>
...
setInterval( function() {
	...
	$('.real_name').hide();
	$('.real_msg').hide();
	changeText($(".real_name"),$("#typed"),150);
	//changeText($(".real_msg"),$("#typed1"),150);
...
<div class="container-fluid">
  <div class="row">
    ...
    <form class="form-horizontal  col-sm-12" action="" method="post" autocomplete="off">

    <div class="col-sm-12 form-cover" style="background:#7fd181;color:#ffffffd4;">
    <div style="width:100%; text-align:center; "> 
    <img src="<?php echo $URLS?>/images/manmesh_logo.jpeg" style="    width: 150px;">
    ...
    Please enter your login details. </p>
	  ...
	  <?php if(isset($msg)){ echo $msg; } ?>
            ...
            <label class="control-label col-sm-12 mob"  for="email">Email Address</label>
            <div class=""><input type="text" name="email"  class="form-control mob-ent" placeholder="Your Email Address">
            </div>
            </div>

            <div class="form-group no-pad position-relative px-5">
            <label class="control-label col-sm-12 mob"  for="email">Password</label>
            <div class=""><input type="text" name="password"  class="form-control mob-ent" placeholder="Your Password">
            </div>
            </div>

            <div class="form-group">
            <button type="submit" name="Login" class="btn btn-default sub_btn">Login</button>
            </div>
